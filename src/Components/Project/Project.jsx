function Project({ 
    ...
    return (
        <section className="grid lg:grid-cols-2 items-center gap-8 relative">
            ...
            <div className={`flex text-center items-center lg:align-baseline lg:items-baseline lg:text-left gap-4 lg:gap-3 flex-col justify-items-start ${comingSoon ? "opacity-15 pointer-events-none select-none" : ""}`}>
                <h2 className="relative text-4xl font-black w-fit">{name}
                    {beta && <span className="absolute -top-2 ml-2 text-lg rotate-6 bg-orange-600 text-white px px-2">Beta</span>}
                    {InProgress && <span className="absolute -top-2 ml-2 text-lg rotate-6 bg-blue-600 text-white px px-2">In&nbsp;Progress</span>}
                </h2>
